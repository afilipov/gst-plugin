...
Hashmap* hashmapCreate(size_t initialCapacity,
    unsigned long (*hash)(void* key), bool (*equals)(void* keyA, void* keyB))
{
    assert(hash != NULL);
    assert(equals != NULL);
 ...
 */
static inline unsigned long hashKey(Hashmap* map, void* key)
{assert(map && key && map->hash);
    ...
    // against bad hashes.
    h += ~(h << 9);
    h ^=  (((unsigned long) h) >> 14);
    h +=  (h << 4);
    h ^=  (((unsigned long) h) >> 10);
